...
namespace BookShare.Application.Services.Implementation
{
    public class AuthenticationService : IAuthenticationService
    {
        ...
        public async Task<StandardResponse<LoginResponseDto>> ValidateAndCreateToken(UserLoginRequestDto requestDto)
        {
            ...
            if (!result)
            {
                return StandardResponse<LoginResponseDto>.Failed("Failed", 401);
            }
            if (!(await _userManager.IsEmailConfirmedAsync(user)))
                return StandardResponse<LoginResponseDto>.Failed("Email not yet confirm. Check your inbox", 403);
            string token = await CreateToken(user);
            UserResponseDto userDto = _mapper.Map<UserResponseDto>(user);
            return StandardResponse<LoginResponseDto>.Success("Successful", new LoginResponseDto { Token=token,User = userDto});
        }

        private void SendConfirmationEmail(string email, string callback_url)
        {
            string logoUrl = "https://www.perkins.org/wp-content/uploads/2017/02/Copy-of-Feature-image-using-app-icon-or-logo-Dark-blue-bg.png";
        ...
        public async Task<StandardResponse<string>> ConfirmEmailAddress(string email, string token)
        {
            ...
            if (user.EmailConfirmed)
                return StandardResponse<string>.Failed("User account confirmed", 401);
        ...
        public async Task<StandardResponse<string>> GenerateEmailActivationToken(string email, HttpRequest httpRequest)
        {
            ...
            var token = await _userManager.GenerateEmailConfirmationTokenAsync(user);
            string encodedToken = System.Text.Encodings.Web.UrlEncoder.Default.Encode(token);
            string callback_url = httpRequest.Scheme + "://" + httpRequest.Host + $"/api/auth/confirm-email/{email}/{encodedToken}";
